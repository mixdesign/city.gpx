...
- (void)applicationWillEnterForeground:(UIApplication *)application
{
    ...
    [self performBlock:^{
        if([User loggedIn])
        {
            NSDate *enterBackgroundDate = [Cache valueForKey:CacheAppEnterBackgroundDate];
            if(enterBackgroundDate) {
                double secondsPassed = [[NSDate date] timeIntervalSinceDate:enterBackgroundDate];
                if(secondsPassed > 600)
                {
                    [UI showAlertWithMessageTitle:@"Внимание" message:@"Сессия работы с мобильным банкингом ХоумКредит истекла. Требуется повторная авторизация."];
                    [UI popToRootAndConfigNavWhite];

                    [AdministrationApi logout:^(id response) {
                        [User logout];
